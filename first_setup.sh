...
CSS_FUNC='css() { local dir="$(pwd)"; while [ "$dir" != "/" ] && [ ! -f "$dir/depart.sh" ]; do dir="$(dirname "$dir")"; done; local suffix=""; if [ "$dir" != "/" ]; then local base_name="$(basename "$dir")"; if echo "$base_name" | grep -qE "^multi-agent-shogun[-_].+"; then suffix="_$(echo "$base_name" | sed -E '\''s/^multi-agent-shogun[-_]//'\'')"; elif [ "$base_name" != "multi-agent-shogun" ]; then suffix="_$base_name"; fi; fi; local target_session="shogun${suffix}"; if ! tmux has-session -t "$target_session" 2>/dev/null; then target_session="shogun"; fi; local s="shogun-$$"; local cols=$(tput cols 2>/dev/null || echo 80); tmux new-session -d -t "$target_session" -s "$s" 2>/dev/null && tmux set-option -t "$s" destroy-unattached on 2>/dev/null; if [ "$cols" -lt 80 ]; then tmux new-window -t "$s" -n mobile 2>/dev/null; tmux attach-session -t "$s:mobile" 2>/dev/null || tmux attach-session -t "$target_session"; else tmux attach-session -t "$s" 2>/dev/null || tmux attach-session -t "$target_session"; fi; }'
CSM_FUNC='csm() { local dir="$(pwd)"; while [ "$dir" != "/" ] && [ ! -f "$dir/depart.sh" ]; do dir="$(dirname "$dir")"; done; local suffix=""; if [ "$dir" != "/" ]; then local base_name="$(basename "$dir")"; if echo "$base_name" | grep -qE "^multi-agent-shogun[-_].+"; then suffix="_$(echo "$base_name" | sed -E '\''s/^multi-agent-shogun[-_]//'\'')"; elif [ "$base_name" != "multi-agent-shogun" ]; then suffix="_$base_name"; fi; fi; local target_session="multiagent${suffix}"; if ! tmux has-session -t "$target_session" 2>/dev/null; then target_session="multiagent"; fi; local s="multi-$$"; local cols=$(tput cols 2>/dev/null || echo 80); tmux new-session -d -t "$target_session" -s "$s" 2>/dev/null && tmux set-option -t "$s" destroy-unattached on 2>/dev/null; if [ "$cols" -lt 80 ]; then tmux new-window -t "$s" -n mobile 2>/dev/null; tmux attach-session -t "$s:mobile" 2>/dev/null || tmux attach-session -t "$target_session"; else tmux attach-session -t "$s" 2>/dev/null || tmux attach-session -t "$target_session"; fi; }'
