...
**Rule: NEVER use `sleep` or `tmux capture-pane` in foreground** — orchestrator blocking halts the entire specialist team. After dispatching → stop and wait for inbox wakeup.

| Command Type | Execution Method |
|-------------|------------------|
| Read / Write / Edit | Foreground |
| inbox_write.sh | Foreground |
| `sleep N` | **FORBIDDEN** — use inbox event-driven |
